...
- Auto billing in `sync_engine.py` creates bills only; it does not auto-create vendor payments. Bill payloads link to Zoho PO line items and must not include `adjustment`/`adjustment_description` for `tax_amount + shipping_amount + handling_amount`, as those charges are already folded into the PO line rates.
...
- Purchase-order outbound payload now includes `cf_source` mapping. Keep mapping aligned with script behavior (`EBAY_* -> Ebay`, `AMAZON_* -> Amazon`, `GOODWILL_SHIPPED` and `GOODWILL_PICKUP` -> `Goodwill`, `ALIEXPRESS_* -> AliExpress`, local pickup variants (`LCPU`, `LOCAL_PICKUP`) -> `Local Pickup`, fallback -> `Other`).
...
- Sales-order source is sent through SO custom fields (`api_name=cf_source`) using `Order.platform` first and `Order.source` only as fallback. Current dropdown mapping: `Ebay_Dragon`, `Ebay_Mekong`, `Ebay_USAV`, `Ebay_Purchasing`, `ECWID`, `Amazon`, `Amazon_Renew`, `Amazon FBA`, `Shopify`, `Walmart`, `Walk-in`, fallback `Other`.
- Sales-order outbound payload checks `Order.fulfillment_channel == AMAZON_FBA` or `Order.platform in {AMAZON_FBA, "AMAZON_FBA"}`; `AMAZON_FBA` orders send `line_items[*].location_id=5623409000001937413` so Zoho assigns the FBA storage location without relying on transaction-level location.
...
- Returns outbound sync uses `ZohoClient.create_sales_return()` against `/salesreturns` only for returned/refunded records after `app.modules.returns.zoho_sync` validates the local order, Zoho Sales Order, line-item mapping, and available quantity. Sales Return create must pass `salesorder_id` as a query param and line payloads must include both product `item_id` and `salesorder_item_id`; the returns service marks matched Zoho items `is_returnable=true` before create. Cancelled/partially cancelled records sync against the Sales Order instead: full cancellation calls `/salesorders/{id}/status/void`, partial cancellation updates line quantities through `/salesorders/{id}`. Do not call sales-return create directly from routes.
...
- Hash-matched sales orders still attempt Zoho fulfilment when local shipping status is `SHIPPING`/`DELIVERED` (or order status is `SHIPPED`) and `zoho_marked_as_fulfilled=false`; never return before this recovery step.
- Sales-order Zoho status lifecycle is `DIRTY` → `QUEUED` → `SYNCING` → `SYNCED` or `ERROR`. `QUEUED` is persisted by force-sync acceptance; set `SYNCED` only after Zoho create/update and confirmation succeed.
- Sales-order outbound sync (`sync_order_outbound`) will block the sync process and set the sync status to `ERROR` if the order contains any unmatched line items (items with `variant_id` as `None` / `status` `UNMATCHED`).
