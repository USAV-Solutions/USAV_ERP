...
- Returns outbound sync uses `ZohoClient.create_sales_return()` against `/salesreturns` only after `app.modules.returns.zoho_sync` validates the local order, Zoho Sales Order, line-item mapping, and available quantity. Do not call sales-return create directly from routes.
