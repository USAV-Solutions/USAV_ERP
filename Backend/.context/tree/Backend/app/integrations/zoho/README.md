...
- Sales-order outbound payload now also checks `Order.fulfillment_channel`; `AMAZON_FBA` orders send `line_items[*].location_id=5623409000001937413` so Zoho assigns the FBA storage location without relying on transaction-level location.
