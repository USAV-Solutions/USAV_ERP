...
- `UserRole` includes `ACCOUNTANT` and `PACKER`; role-expansion requires database enum migration before assigning that role in persisted user records.
...
- `Customer.amazon_buyer_id` is the stable Amazon FBA customer key used for import matching and Zoho contact naming. Keep local `Customer.name` human-readable; do not repurpose it as the canonical FBA identity.
