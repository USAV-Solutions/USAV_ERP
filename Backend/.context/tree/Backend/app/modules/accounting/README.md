...
- Export endpoints use `file_type` (`csv` or `xlsx`) and return attachment streams; they now also accept `export_full=true` to export ungrouped order rows for the selected date/filter window, including `inventory_name` (from `ProductVariant.variant_name`) while `item` remains the original order-side item title.
...
- Sales report export endpoint `/accounting/reports/sales-orders/export` supports `file_type` (`csv` or `xlsx`) and `export_full=true` for ungrouped rows; in full-export mode it emits `inventory_name` (`ProductVariant.variant_name`), separate `inventory_sku` (from `ProductVariant.full_sku`, blank for unmatched items), and `platform_sku` (listing/order external reference id) columns.
