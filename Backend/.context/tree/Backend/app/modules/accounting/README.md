...
- Sales report export endpoint `/accounting/reports/sales-orders/export` supports `file_type` (`csv` or `xlsx`) and `export_full=true` for ungrouped rows; in full-export mode it emits `inventory_name` (`ProductVariant.variant_name`), separate `inventory_sku` (from `ProductVariant.full_sku`, blank for unmatched items), `platform_sku` (listing/order external reference id), and `tracking_number` columns.
