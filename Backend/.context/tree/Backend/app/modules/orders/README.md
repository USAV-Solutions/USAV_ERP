...
- Forgetting to align platform/source enums across model, schema, migration, and frontend types. Platform enum now includes `AMAZON_RENEW`, `AMAZON_FBA`, and `WALK_IN` alongside existing platforms.
...
- `CSV_GENERIC` order import now treats ShipStation and eBay Orders Report CSVs as line-item rows grouped by `Order - Number`; order-level fields (totals, ship-to, customer, date) are read from the first row and item fields come from `Item - Name`/`Item - SKU`/`Item - Qty`/`Item - Price` (or eBay equivalents like `Item Title`, `Custom Label`, `Quantity`, `Sold For`). For ShipStation rows, missing line totals now fall back to `Item - Price * Qty` instead of order-level subtotal/total so marketplace line items do not absorb tax/shipping. eBay total amounts map correctly by parsing string price values ($x.xx) to numerical float, and date format parsing supports eBay `Mmm-dd-yy`.
...
- `CSV_GENERIC` import now attempts per-row platform detection (`platform`/`source` plus ShipStation store/source columns) and ingests batches under detected `orders.platform` values (`WALK_IN`, `SHOPIFY`, `ECWID`, `WALMART`, `AMAZON_RENEW`, `AMAZON_FBA`, `AMAZON`, `EBAY_DRAGON`, `EBAY_MEKONG`, `EBAY_PURCHASING`, `EBAY_USAV`). Rows identified as `ECWID` are skipped so ShipStation CSV cannot duplicate Ecwid API orders. Matching prioritizes `DRAGON` before generic `EBAY` so eBay Dragon orders are no longer misclassified as `EBAY_USAV`.
...
- `SHIPPING_STATUS_CSV` file import source: Allows bulk updating order shipping status via a CSV (matching by `order_number` column and applying `scraped_status`). Both `SHIPPED` and `SHIPPING` map to `SHIPPING`; unchanged shipping statuses are skipped, while updated orders are marked with Zoho sync status `DIRTY` so Range Sync pushes updates to Zoho. Unrecognised/empty statuses default to `PENDING`. The status→`ShippingStatus` mapping is `app/modules/tracking/status_mapping.map_scraped_status`, shared with the server-side tracking scraper (`app/modules/tracking/`), which is the preferred path — it scrapes parcelsapp on the server instead of uploading a CSV.
- `orders.tracking_last_checked_at` records when the tracking scraper last checked an order; the scraper's eligible-order query skips orders checked within `tracking_freshness_hours` so a job can safely resume.
