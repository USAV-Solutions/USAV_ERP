...
- Return records match local sales orders by platform plus external order ID or external order number. Records that still cannot be linked during ingestion receive the `MISSING_LOCAL_ORDER` `zoho_sync_status` while preserving their original normalized status; they can be re-attempted manually via the rematch endpoint which searches the linked external order ID and links the record, reverting `zoho_sync_status` to `PENDING`.
- Amazon return/cancellation data is imported via CSV (`POST /returns/import-amazon-csv`). The importer accepts the old cancelled-orders export, merchant return report TSV (`Order ID`, `Return quantity`, `Amazon RMA ID`), and FBA return report CSV (`return-date`, `order-id`, `quantity`, `license-plate-number`). Upload decode tries UTF-8 first, then cp1252/latin-1 because Amazon exports can contain non-UTF-8 product-name bytes.
