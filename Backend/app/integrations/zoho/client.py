...
class ZohoClient:
    ...
    async def mark_bill_open(self, bill_id: str) -> dict:
        """Mark a bill as open in Zoho Inventory."""
        return await self._request("POST", f"/bills/{bill_id}/status/open")

    async def mark_bill_void(self, bill_id: str) -> dict:
        """Mark a bill as void in Zoho Inventory."""
        return await self._request("POST", f"/bills/{bill_id}/status/void")

    async def delete_bill_payment(self, bill_payment_id: str) -> dict:
        """Delete a bill payment using the bill payment id."""
        return await self._request("DELETE", f"/billpayments/{bill_payment_id}")

    async def delete_vendor_payment(self, payment_id: str) -> dict:
        """Delete a vendor payment using the payment id."""
        return await self._request("DELETE", f"/vendorpayments/{payment_id}")

    async def delete_bill_payment_reference(self, payment: dict[str, Any]) -> bool:
        """
        Delete a bill-linked payment using whichever payment identifier endpoint is accepted.

        Zoho bill payloads include both `bill_payment_id` and `payment_id` depending on
        account/config state. We try both documented endpoint styles.
        """
        bill_payment_id = str(payment.get("bill_payment_id") or "").strip()
        payment_id = str(payment.get("payment_id") or "").strip()

        bill_id = str(payment.get("bill_id") or "").strip()
        attempts: list[tuple[str, Any]] = []
        if bill_payment_id:
            attempts.append(("bill_payment_id", lambda: self.delete_bill_payment(bill_payment_id)))
        if payment_id:
            attempts.append(("payment_id", lambda: self.delete_vendor_payment(payment_id)))
        if bill_id and bill_payment_id:
            attempts.append(
                (
                    "bill_id+bill_payment_id",
                    lambda: self._request("DELETE", f"/bills/{bill_id}/payments/{bill_payment_id}"),
                )
            )
        if bill_id and payment_id:
            attempts.append(
                (
                    "bill_id+payment_id",
                    lambda: self._request("DELETE", f"/bills/{bill_id}/payments/{payment_id}"),
                )
            )

        for id_kind, deleter in attempts:
            try:
                await deleter()
                logger.info(
                    "Zoho delete_bill_payment_reference succeeded | id_kind=%s bill_payment_id=%s payment_id=%s",
                    id_kind,
                    bill_payment_id,
                    payment_id,
                )
                ...
                # Ignore expected not-found style responses and continue fallback.
                if "not found" in message or '"code":1002' in message:
                    continue
                logger.warning(
                    "Zoho delete_bill_payment_reference failed | id_kind=%s bill_payment_id=%s payment_id=%s error=%s",
                    id_kind,
                    bill_payment_id,
                    payment_id,
                    exc,
    ...
    async def list_purchase_receives(
        ...
        *,
        ...
        """List purchase receives for a purchase order."""
        result = await self._request(
            "GET",
            "/purchasereceives",
            params={
        ...
        return result.get("purchase_receives", [])
